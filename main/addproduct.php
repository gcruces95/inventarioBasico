<?php
    ...
    if($_POST['marca'] != null && $_POST['modelo'] != null && $_POST['procesador'] != null){
        ...
        if ($result->num_rows == 0) {
            ...
            $query = "INSERT INTO procesador (slug, nombre, modelo_id, marca_id) VALUES ('$procesador_slug', '$procesador_nombre', '$modelo_id', '$marca_id')";
            if (!$conn->query($query)) {
                echo "<script>alert('Error: No se pudo agregar la marca')</script>";
                header("Refresh:0 , url = ../addproduct.php");
                die("Error: No se pudo agregar la marca");
            }
            $procesador_id = $conn->insert_id; // Obtenemos el ID de la marca recién insertada
        } else {
            ...
            // La marca ya existe, obtenemos su ID
            $row = $result->fetch_assoc();
            $procesador_id = $row['id'];
        }

        echo "<script>alert('Equipo agredado correctamente')</script>";
        header("Refresh:0 , url = ../addproduct.php");
    }
